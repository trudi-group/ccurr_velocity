...
\begin{frame}[t]{Simple mod}
  % \vfill

  \only<1>{
    \begin{block}{Fisher's Equation of Exchange (Fisher, 1911)}
      \[ \scalebox{1.5}{$PT=MV$
          \phantom{$\frac{I}{I}$}} \]  
    \end{block}
  }
  ...
  \only<2>{
    ...
    \begin{figure}
      \centerline{\includegraphics[scale=0.11]{./pics/used/paper-trail.png}}
    \end{figure}
  }
  \only<3>{
    \begin{itemize}
...
\begin{frame}{Recently proposed measures}
  ...
  \only<3>{
    ...
    Theoretically proposed in \cite{bolt2016value}, operationalized by us :)
    \smallskip{}
    \begin{align}%
...
\begin{frame}{Counting Coins ...}
    \begin{figure}
      \centerline{\includegraphics[scale=0.23]{./pics/used/mcirc_concept_window_uneqal_period.png}}
    \end{figure}
\end{frame}
